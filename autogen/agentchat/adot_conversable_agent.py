...
class AdotConversableAgent(Agent):
    ...
    def __init__(
        ...
        self.register_reply([Agent, None], AdotConversableAgent.generate_oai_reply)
        self.register_reply([Agent, None], AdotConversableAgent.generate_code_execution_reply)
        self.register_reply([Agent, None], AdotConversableAgent.generate_function_call_reply)
        self.register_reply([Agent, None], AdotConversableAgent.check_termination_and_human_reply)

        # a dictionary of conversations, default value is list
        if database is None:
            raise ValueError("Database must be included.")

        self._oai_messages = database
        self._oai_agents = defaultdict(list)
    ...
    def chat_messages(self, session_id:str, agent: Optional[Agent] = None) -> Dict[Agent, List[Dict]]:
        """A dictionary of conversations from agent to list of messages."""
        return self._oai_messages.query(session_id=session_id, agent=agent)

    def last_message(self, session_id:str, agent: Optional[Agent] = None) -> Dict:
        ...
        """
        if agent is None:
            n_conversations = len(self._oai_agents)
            if n_conversations == 0:
                return None
            if n_conversations == 1:
                return self._oai_messages.query(session_id=session_id, limit=1)
            raise ValueError("More than one conversation is found. Please specify the sender to get the last message.")
        return self._oai_messages.query(session_id=session_id, agent=agent, limit=1)
    ...
    def _append_oai_message(self, session_id:str, message: Union[Dict, str], role, conversation_id: Agent) -> bool:
        ...
        self._oai_messages.add(session_id=session_id, agent=conversation_id, item=oai_message)
        return True
    ...
    def send(
        ...
        valid = self._append_oai_message(session_id, message, "assistant", recipient)
        if valid:
            return_message = recipient.receive(session_id, message, self, request_reply, silent)
        else:
            raise ValueError(
    ...
    async def a_send(
        ...
        valid = self._append_oai_message(session_id, message, "assistant", recipient)
        if valid:
            await recipient.a_receive(session_id, message, self, request_reply, silent)
        else:
            raise ValueError(
                "Message can't be converted into a valid ChatCompletion message. Either content or function_call must be provided."
            )
    ...
    def _process_received_message(self, session_id, message, sender, silent):
        message = self._message_to_dict(message)
        # When the agent receives a message, the role of the message is "user". (If 'role' exists and is 'function', it will remain unchanged.)
        valid = self._append_oai_message(session_id, message, "user", sender)
        if not valid:
            raise ValueError(
    ...
    def receive(
        ...
        Raises:
            ValueError: if the message can't be converted into a valid ChatCompletion message.
        """
        self._process_received_message(session_id, message, sender, silent)
        if request_reply is False or request_reply is None and self.reply_at_receive[sender] is False:
            return
        final, reply = self.generate_reply(session_id=session_id, messages=self.chat_messages(session_id=session_id, agent=sender), sender=sender)
        if final == 'ADOT' and reply is not None:
            return reply
        elif reply is not None:
            return self.send(session_id, reply, sender, silent=silent)
    ...
    async def a_receive(
        ...
        if request_reply is False or request_reply is None and self.reply_at_receive[sender] is False:
            return
        final, reply = await self.a_generate_reply(session_id=session_id, sender=sender)
        if final == 'ADOT' and reply is not None:
            return reply
        elif reply is not None:
            return await self.a_send(session_id, reply, sender, silent=silent)
    ...
    def initiate_chat(
        ...
        **context,
        ...
        return_message = self.send(session_id, self.generate_init_message(**context), recipient, silent=silent)
        return return_message
    ...
    async def a_initiate_chat(
        ...
        **context,
        ...
        """
        self._prepare_chat(recipient, clear_history)
        return_message = await self.a_send(session_id, self.generate_init_message(**context), recipient, silent=silent)
        return return_message
    ...
    def clear_history(self, agent: Optional[Agent] = None):
        ...
        """
        return
        raise ValueError("clear_history 이거는 허용 안함.")
        ...
        **kwargs,
        ...
        if self.human_input_mode == "ALWAYS":
            ...
            if self._consecutive_auto_reply_counter[sender] >= self._max_consecutive_auto_reply_dict[sender]:
                ## 테스트용
                if self.human_input_mode == "ADOT":
                    return "ADOT", message
                elif self.human_input_mode == "NEVER":
                    reply = "exit"
                else:
    ...
    def generate_reply(
        ...
        for reply_func_tuple in self._reply_func_list:
            ...
            if self._match_trigger(reply_func_tuple["trigger"], sender):
                final, reply = reply_func(self, session_id=session_id, messages=messages, sender=sender, config=reply_func_tuple["config"])
                if final:
                    return final, reply
        return False, self._default_auto_reply
