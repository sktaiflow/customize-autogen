...
from autogen.adot_utils import create_vector_db_from_dir, query_vector_db, num_tokens_from_text
...
You must give answer in korean.
...
class RetrieveUserProxyAgent(UserProxyAgent):
    ...
    def _get_context(self, results: Dict[str, Union[List[str], List[List[str]]]]):
        doc_contents = "\n---------------\n" if len(results["documents"][0]) >0 else ""
        current_tokens = 0
